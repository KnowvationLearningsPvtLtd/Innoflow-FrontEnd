...
export function PromptNode({ data, isConnectable }: { data: any; isConnectable?: boolean }) {
  const [template, setTemplate] = useState(data.template || "Write a {{style}} about {{topic}}...")
  ...
  return (
    <div className="min-w-[240px] rounded-md border border-yellow-500/40 bg-black/80 shadow-lg backdrop-blur-sm glow glow-yellow">
      <div className="border-b border-yellow-500/30 bg-yellow-500/10 px-4 py-2 text-sm font-medium text-yellow-500 flex items-center gap-2">
        <div className="flex h-5 w-5 items-center justify-center rounded bg-yellow-500/20 text-xs text-yellow-500">
          P
        </div>
        <span>{data.label || "Prompt"}</span>
      </div>
      <div className="p-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-2">
                {variables.map((variable, index) => (
                  <div key={index} className="rounded-full bg-yellow-500/20 px-2 py-1 text-xs text-yellow-500">
                    {variable}
                  </div>
                ))}
              </div>
